...
if ~isempty(varargin),
    switch varargin{1},
        case 'js',
            pVect1_nz_id = pVect1 > 0;
            pVect2_nz_id = pVect2 > 0;
            pVect_nz_id = pVect1_nz_id & pVect2_nz_id;
            logQvect = log((pVect2+pVect1)/2);
            KL = .5 * (sum(pVect1(pVect_nz_id).*(log(pVect1(pVect_nz_id))-logQvect(pVect_nz_id))) + ...
                sum(pVect2(pVect_nz_id).*(log(pVect2(pVect_nz_id))-logQvect(pVect_nz_id))));
            
            
%             logQvect = log((pVect2+pVect1)/2 + eps);
%             KL = .5 * (sum(pVect1.*(log(pVect1 + eps)-logQvect)) + ...
%                 sum(pVect2.*(log(pVect2 + eps)-logQvect)));

        case 'sym',
            pVect1_nz_id = pVect1 > 0;
            pVect2_nz_id = pVect2 > 0;
            pVect_nz_id = pVect1_nz_id & pVect2_nz_id;
            KL1 = sum(pVect1(pVect_nz_id) .* (log(pVect1(pVect_nz_id))-log(pVect2(pVect_nz_id))));
            KL2 = sum(pVect2(pVect_nz_id) .* (log(pVect2(pVect_nz_id))-log(pVect1(pVect_nz_id))));
            KL = (KL1+KL2)/2;
            

%             KL1 = sum(pVect1 .* (log(pVect1 + eps)-log(pVect2 + eps)));
%             KL2 = sum(pVect2 .* (log(pVect2 + eps)-log(pVect1 + eps)));
%             KL = (KL1+KL2)/2;
...
%     pVect1_nz_id = pVect1 > 0;
    KL = sum(pVect1 .* (log(pVect1 + eps)-log(pVect2 + eps)));
end